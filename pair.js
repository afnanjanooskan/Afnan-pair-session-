...
router.get("/qr", async (req, res) => {
  const sessionDir = `./session_qr_${Date.now()}_${Math.random().toString(36).slice(2)}`;
  const { state, saveCreds } = await useMultiFileAuthState(sessionDir);
  ...
  try {
    ...
    RobinQR.ev.on("connection.update", async (s) => {
      ...
      // Send QR code as a base64 data URL image
      if (qr && !qrSent && !res.headersSent) {
        qrSent = true;
        try {
          const qrDataUrl = await qrcode.toDataURL(qr);
          return res.send({ qr: qrDataUrl });
        } catch (e) {
          if (!res.headersSent) res.send({ error: "Failed to generate QR" });
        }
      }
    ...
    await removeFile(sessionDir);
    if (!res.headersSent) {
      return res.send({ error: "Service Unavailable" });
    }
  }
});
